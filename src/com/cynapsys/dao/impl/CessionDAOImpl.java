...
//	}
	...
	public List<Cession> findCessionbyDate(Date dd, Date df , Employeur emp){
		SimpleDateFormat format = new SimpleDateFormat("dd/MM/yyyy");
		 ...
		 req.append("from Cession s left join fetch s.employeur left join fetch s.cessionClient d left join fetch d.clientAcquereur  where s.employeur ="
		 		...
		 		+ format.format(dd)+"' and '" 
		 		+ format.format(df)+"' ) or (('"+format.format(dd)+"' between s.dateDebut and s.dateFin) and ('"+format.format(df)+"' between s.dateDebut and s.dateFin )))");	
	...
	@Override
	public List<Cession> findCessionByemployeurEcheancier(Integer date) {
		List<Cession> list = new ArrayList<Cession>();
		date=1900+date;
		StringBuilder req = new StringBuilder();
		req.append(
				" select  distinct new Cession(I.id,I.dateDebut,I.dateFin,I.mensualite,k.codeEmployeur,k.nomEmployeurFr)from  Cession as I   ");
		req.append("  inner join I.employeur as k  ");
		req.append(" where ");
		

		req.append (" (  to_number(to_char(I.dateDebut,'yyyy') ,'99999') < ").append(date+1);
		req.append(") and (  to_number(to_char(I.dateFin,'yyyy') ,'99999') > ").append(date-1);
		req.append(")");
		...
		req.append(" order by k.codeEmployeur , I.dateDebut ASC ");
