...
public class GestionArrangementBean implements Serializable {
	...
	private Integer dureeArra=null;
	private Integer mensulaiteArr=null;
	...
	public void onSelectArr() {
		...
		dureeArra=(int) newArrangement.getDuree();
		mensulaiteArr=(int) newArrangement.getNombreMensualite();
	}
	...
	public void saveArrangement(){
		...
		if(modifArrangement==0){
			...
			if(mensulaiteArr>=(newArrangement.getMontantArrieres().intValue()/2)){
				//newArrangement.setContentieux(newArrangement.getPrecontentieu().getContentieux());
				newArrangement.setDateFin(addMonth(newArrangement.getDateDebut(), dureeArra));
				newArrangement.setDuree(dureeArra);
				newArrangement.setNombreMensualite(mensulaiteArr);
				arrangementService.save(newArrangement);
				imprimerArrangement(newArrangement);
				//ClientAcquereur clientAcquereur;
				newArrangement=arrangementService.findById(newArrangement.getId().toString());
				//clientAcquereur=newArrangement.getPrecontentieu().getImpayee().getClientAcquereur();
				//clientAcquereur.setEnArrangement(true);
				//newArrangement.getPrecontentieu().getImpayee().getClientAcquereur().setEnArrangement(true);
				clientRech.setEnArrangement(true);
				clientAcquereurService.update(clientRech);
			}
			else{
				JsfUtil.addErrorMessage("Montant doit etre supérieure à la moitié du montant des arriérées");
			}
			...
			newArrangement.setContentieux(newArrangement.getPrecontentieu().getContentieux());
			newArrangement.setDateFin(addMonth(newArrangement.getDateDebut(), newArrangement.getDuree()));
			newArrangement.setDuree(dureeArra);
			newArrangement.setNombreMensualite(mensulaiteArr);
			arrangementService.update(newArrangement);
			imprimerArrangement(newArrangement);
		}
	...
	public void onChangeDateDebArr(){
		
		dateF=addMonth(dateF,newArrangement.getDuree());
	...
	public void imprimerArrangement(Arrangement arrangement)
	{
           ...
           //if(arrangement.getPrecontentieu().getImpayee().getClientAcquereur().getNomPrenomAr()==null && !arrangement.getPrecontentieu().getImpayee().getClientAcquereur().getNomPrenomAr().isEmpty()){
        	   data.put("impNom", clientRech.getNomPrenomFr());
           //}
           //else{
        	  // data.put("impNom", arrangement.getPrecontentieu().getImpayee().getClientAcquereur().getNomPrenomFr());
           //}
	       
	       data.put("numLoc",clientRech.getLogement().getCodeLogement());
	       data.put("localAdre",clientRech.getLogement().getAdresseLoge());
	       ...
	       data.put("dateArr",arrangement.getDateDebut().toString().substring(0, 10));
	       data.put("montantRest",String.valueOf(arrangement.getMontantArrieres().intValue()-arrangement.getNombreMensualite()));
	       data.put("montantTranche",String.valueOf((arrangement.getMontantArrieres().intValue()-arrangement.getNombreMensualite())/arrangement.getDuree()));
	...
	public Date addMonth(Date date, Integer months)
	{
...
//		etatFilter[3] = new SelectItem(groupes.get(2).getLibelle(),groupes.get(2).getLibelle());
	...
	public int getModifArrangement() {
		return modifArrangement;
	}

	public void setModifArrangement(int modifArrangement) {
		this.modifArrangement = modifArrangement;
	}

	public Integer getDureeArra() {
		return dureeArra;
	}

	public void setDureeArra(Integer dureeArra) {
		this.dureeArra = dureeArra;
	}

	public Integer getMensulaiteArr() {
		return mensulaiteArr;
	}

	public void setMensulaiteArr(Integer mensulaiteArr) {
		this.mensulaiteArr = mensulaiteArr;
	}
